...
// Block 1: .password-modal-overlay
...
if (content.includes(overlayBlock)) {
  ...
  changed = true;
  console.log('Replaced .password-modal-overlay @extend block');
}

if (content.includes(modalBlock)) {
  content = content.replace(modalBlock, modalStyles);
  changed = true;
  console.log('Replaced .password-modal @extend block');
}

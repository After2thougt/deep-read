...
import { BookOpen, Check, ChevronLeft, ChevronRight, Pencil, Plus, Trash2, X } from "lucide-react";
...
function TagPill({ tag, active, onClick, onPrefetch, managing, onRename, onDelete }) {
  return <div className={`tag-pill-wrapper ${managing ? "is-managing" : ""}`}>
    ...
    {managing && <button type="button" className="tag-delete-button" onClick={() => onDelete(tag)} aria-label={`Delete ${tag.name}`}><X size={12} /></button>}
  </div>;
}
...
export default function ArticlesPage({ onOpenArticle }) {
  ...
  const loadArticles = useCallback(async () => {
    try {
      ...
      const result = await fetchArticles({ page, limit: PAGE_SIZE, tag: selectedTag });

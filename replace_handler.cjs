...
const startIdx = content.indexOf('POST /api/articles/images/from-url');

// Find the end - the }); followed by blank line then app.get (handle \r\n)
const endSearchStart = startIdx + 3300;
let endIdx = content.indexOf('});\r\n\r\napp.get', endSearchStart);
if (endIdx === -1) {
...
const newContent = content.substring(0, startIdx) + newHandler + content.substring(endIdx);

fs.writeFileSync('D:/Projects/deep-read/backend/server.js', newContent, 'utf8');
console.log('Replacement done! New file size:', newContent.length);